...
| `tests/test_crud.py` | add / delete / delete_nodes / get_nodes / clear | 11 |
| `tests/test_search.py` | DEFAULT / SPARSE / HYBRID / MMR 四种查询模式 | 12 |
| `tests/test_filter.py` | 14 种 FilterOperator + AND/OR + GIN 数组操作 | 18 |
| `tests/test_async.py` | async_add / adelete / adelete_nodes / aget_nodes / aclear / aquery | 12 |
| `tests/test_integration.py` | E2E 生命周期、Hybrid 端到端、IndexNode、多实例隔离 | 7 |
...
| 10 | `test_add_to_db_query_and_delete` | `test_add_and_delete_by_ref_doc_id` | DELETE SQL → client.delete(expr=...) |
| 11 | `test_sparse_query` | `test_sparse_query` | to_tsquery SQL → expr 字符串 |
| 12 | `test_hybrid_query` | `test_hybrid_query` / `test_hybrid_query_hnsw` | 双路并发 + Python 侧去重合并 |
...
| 16 | `test_clear` | `test_clear_collection` | DELETE FROM → truncate_collection |
...
| 上游测试 | 移除原因 |
|---------|---------|
| `test_custom_engines` | VastbaseVectorStore 不接受外部 SQLAlchemy engine (D-05 决策) |
...
| `test_collection_name_format` | 验证 _collection_name 格式 (schema.data_table) |
...
# 期望结果：全部 45 个测试成功收集，无 import 错误

$ python -m pytest tests/ -v
# 期望结果：全部 FAIL — 因 VastbaseVectorStore 尚未实现
```
