...
def test_from_params_custom():
    ...
    assert store.hnsw_kwargs == {"hnsw_m": 32, "hnsw_ef_construction": 128, "hnsw_ef_search": 80}
    asyncio.run(store.close())
...
@pytest.mark.skipif(vastbase_not_available, reason="Vastbase is not available")
def test_indexed_metadata_initialization(vb_indexed_metadata: VastbaseVectorStore):
...
@pytest.mark.skipif(vastbase_not_available, reason="Vastbase is not available")
def test_gin_array_indexed_initialization(vb_gin_array_indexed: VastbaseVectorStore):
...
def test_collection_name_format():
    """Verify internal _collection_name follows schema.table_name convention."""
    store = VastbaseVectorStore.from_params(
        host="172.16.105.107",
        port=15432,
        database="vastbase",
    ...
    assert store._collection_name == "my_schema.data_my_table"
    asyncio.run(store.close())
