...
# Session-scoped store that persists across tests for the instance-creation check
@pytest.fixture()
def vb_session() -> Generator[VastbaseVectorStore, None, None]:
    """Standalone VastbaseVectorStore for instance creation test (no per-setup needed)."""
    store = VastbaseVectorStore.from_params(
        host=VASTBASE_HOST,
        port=VASTBASE_PORT,
        database=VASTBASE_DATABASE,
        user=VASTBASE_USER,
        password=VASTBASE_PASSWORD,
        table_name=TEST_TABLE_NAME + "_session",
        schema_name=TEST_SCHEMA_NAME,
    )
    yield store
    asyncio.run(store.close())
